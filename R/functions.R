...
create_clock <- function(hour, minute) {
  ...
  clock_data <- data.frame(
    x = c(0, 0),
    y = c(0, 0),
    xend = c(0.15 * cos(hour_angle), 0.22 * cos(minute_angle)),  # Slightly reduce hand length
    yend = c(0.15 * sin(hour_angle), 0.22 * sin(minute_angle)),
    hand = c("hour", "minute")
  )
  ...
  tick_data <- data.frame(
    x = 0.26 * cos(tick_angles),
    y = 0.26 * sin(tick_angles),
    xend = 0.22 * cos(tick_angles),  # Short inward-facing dashes
    yend = 0.22 * sin(tick_angles)
  )
  
  # Create ggplot clock
  ggplot() + 
    # Circle for clock face (smaller size)
    geom_point(aes(x = 0, y = 0), size = 40, shape = 1) +  # Adjust circle size
    
    # Small inner circle at the center of the clock
    geom_point(aes(x = 0, y = 0), size = 4, shape = 1) +  # Smaller inner circle for hands attachment
    
    # Clock ticks as inward-facing dashes
    geom_segment(data = tick_data, aes(x = x, y = y, xend = xend, yend = yend), size = 0.5) + 
