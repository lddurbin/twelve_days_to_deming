...
- **What** — Added an optional `session_minutes:` front-matter field surfaced by `filters/reading-time.lua` as a second number alongside the reading-time estimate (`~ N min reading · ~ M min full session`). Encoded the field on Day 1's clean-clock chapters (10 of 13 chapters) using the gap between consecutive clock anchors as the budget. The filter clamps the dual indicator to fall back to the legacy single-number output when `session_minutes` would be shorter than the prose reading estimate at 200 wpm (currently affects `04-resources` and `09-different`, where Neave's gap is briefer than our reading metric). Left `session_minutes` unset on Day 5 and Day 8 despite Neave's source having clock data on those days, because the data is editorially unreliable.
...
- **Why** — Encoding clock budgets where they are reliable surfaces Neave's own pacing signal more faithfully than the prose-only reading-time estimate; the issue evidence shows the ratio runs **3–5×** for activity-heavy chapters. The decision to *omit* the field on Days 5 and 8 — rather than emit a calculated number from the broken/insufficient source — is the deviation: we are silently dropping Neave's published clocks where the underlying data does not justify carrying them through. The dual-number indicator only renders when `session_minutes` is set, so Days 2–12 fall back to the existing `~ N min reading [+ activities]` output unchanged.
- **Decided in** — Issue [#287](https://github.com/lddurbin/twelve_days_to_deming/issues/287).
- **Landed in** — *Pending — tracked in [#287](https://github.com/lddurbin/twelve_days_to_deming/issues/287).*
