...
#| fig-alt: "Three smooth bell curves overlaid on a single panel. All three are centred on the same mean μ but have progressively wider spreads — a tall narrow curve labelled 'Smallish σ', a medium curve labelled 'Larger σ', and a short wide curve labelled 'Still larger σ'. Successive multiples of σ from μ−3σ to μ+3σ are marked on the x axis (with σ taken to be the middle curve's σ for tick spacing)."
#| fig-width: 8
#| fig-height: 4
sigma_tick <- 2  # use the middle curve's σ for tick spacing
pdf_family_plot(
  pdfs = list(
    "Smallish σ"     = function(x) dnorm(x, mean = 0, sd = 1),
    "Larger σ"       = function(x) dnorm(x, mean = 0, sd = 2),
    "Still larger σ" = function(x) dnorm(x, mean = 0, sd = 3)
  ),
  xlim = c(-3, 3) * sigma_tick + c(-1, 1)
) +
  scale_x_continuous(
    breaks = seq(-3, 3) * sigma_tick,
    labels = c("μ−3σ", "μ−2σ", "μ−σ", "μ", "μ+σ", "μ+2σ", "μ+3σ"),
    limits = c(-3, 3) * sigma_tick + c(-1, 1),
    expand = c(0, 0)
  )
```
...
#| fig-alt: "A single normal curve with the area under it divided into shaded regions and labelled with the standard percentages. The central inner ±σ band is shaded blue and labelled 34.1% on each side of μ. The next outward band from ±σ to ±2σ is shaded red and labelled 13.6% on each side. The band from ±2σ to ±3σ is shaded mid-grey and labelled 2.15% on each side. The two extreme tails beyond ±3σ are shaded a darker red and labelled 0.135% via leader arrows. Successive multiples of σ from μ−3σ to μ+3σ are marked on the x axis."
#| fig-width: 7
#| fig-height: 4.5
xx <- seq(-4, 4, length.out = 1001)
dens <- data.frame(x = xx, y = dnorm(xx))

ribbon_band <- function(lo, hi, fill, alpha) {
  sub <- dens[dens$x >= lo & dens$x <= hi, ]
...
ggplot(dens, aes(x = .data$x, y = .data$y)) +
  # Inner ±σ band: 34.1% on each side of μ.
  ribbon_band(-1, 1, CONTROL_LIMIT_COLOUR, 0.25) +
  # Next band ±σ to ±2σ: 13.6% each side.
  ribbon_band(-2, -1, CHART_LINE_COLOUR, 0.30) +
  ribbon_band( 1,  2, CHART_LINE_COLOUR, 0.30) +
  # Band ±2σ to ±3σ: 2.15% each side.
  ribbon_band(-3, -2, CHART_GRID, 0.50) +
  ribbon_band( 2,  3, CHART_GRID, 0.50) +
  # Tails beyond ±3σ: 0.135% each side.
  ribbon_band(-4, -3, CHART_LINE_COLOUR, 0.70) +
  ribbon_band( 3,  4, CHART_LINE_COLOUR, 0.70) +
  # Curve over the top.
  geom_line(colour = CHART_FG, linewidth = 0.6) +
  # Drop a thin vertical line at the mean for visual separation of the
  # two 34.1% labels — matches Neave's printed figure.
  annotate("segment", x = 0, xend = 0, y = 0, yend = dnorm(0),
           colour = CHART_FG, linewidth = 0.3) +
  # Percentage labels in the body of the curve.
  annotate("text", x = -0.5, y = 0.18, label = "34.1%",
           colour = CHART_FG, size = 4) +
  annotate("text", x =  0.5, y = 0.18, label = "34.1%",
           colour = CHART_FG, size = 4) +
  annotate("text", x = -1.5, y = 0.07, label = "13.6%",
           colour = CHART_FG, size = 3.5) +
  annotate("text", x =  1.5, y = 0.07, label = "13.6%",
           colour = CHART_FG, size = 3.5) +
  annotate("text", x = -2.45, y = 0.018, label = "2.15%",
           colour = CHART_FG, size = 3.2) +
  annotate("text", x =  2.45, y = 0.018, label = "2.15%",
           colour = CHART_FG, size = 3.2) +
  # Tail labels with leader arrows pointing to the (very small) tail region.
  annotate("segment", x = -3.7, xend = -3.15, y = 0.03, yend = 0.005,
           colour = CHART_FG, linewidth = 0.3,
           arrow = arrow(length = unit(0.12, "cm"))) +
  annotate("text", x = -3.7, y = 0.035, label = "0.135%",
