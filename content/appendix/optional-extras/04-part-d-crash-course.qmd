...
pagetitle: "Optional Extras Part D: a crash course in conventional Statistics"
description: "Histograms, probability distributions, the normal distribution, the Central Limit Theorem, confidence intervals, and hypothesis tests, in plain terms."
execute:
  echo: false
---
...
[]{#sec-optional-extras-page-39}Now suppose we toss the two coins several times, keeping track of how often we get no Heads, one Head, and two Heads. Here are some typical results. After tossing the coins ten times, we might have:
...
Portrayal of the probabilities of all possible individual outcomes in the situation being considered, either as a picture like that on [page 38](04-part-d-crash-course.qmd#sec-optional-extras-page-38) or just the numbers on which that picture is based, leads us to the idea of a *probability distribution*. That coin-tossing case of
...
So, when we have continuous data, can we copy what we did with discrete data such as when we were examining the number of Heads when two coins are tossed? That is, can we collect various amounts of data, form histograms of those data as we did on [page 39](04-part-d-crash-course.qmd#sec-optional-extras-page-39), and see those histograms approaching a picture of a probability distribution such as the one we saw on [page 38](04-part-d-crash-course.qmd#sec-optional-extras-page-38)? The answer is partly Yes and partly No. The devil, as people say, is in the detail. There are both some important similarities and some important differences in the continuous case compared with the discrete case.
...
```{r}
#| label: fig-temps-10k-int
#| fig-cap: "Body-temperature histogram: 10,000 readings to the nearest integer"
#| fig-alt: "Bar histogram of 10,000 body-temperature readings rounded to the nearest integer degree Celsius. The three visible bars (36, 37, 38) have similar relative heights to the 1,000-reading version, with the normal curve overlaid to hint at the underlying smooth distribution."
#| fig-width: 7
#| fig-height: 5
set.seed(353)
v <- round(rnorm(10000, mean = 36.8, sd = 0.35), 0)
histogram_with_pdf(v, binwidth = 1, boundary = 0.5)
```
...
```{r}
#| label: fig-temps-1k-1dp
#| fig-cap: "Body-temperature histogram: 1,000 readings to one decimal place"
#| fig-alt: "Bar histogram of 1,000 body-temperature readings rounded to one decimal place. About 20 bars span roughly 35.9°C to 37.8°C — a clearly bell-shaped but jagged outline that approximates a normal density. A smooth blue normal curve is overlaid for comparison."
#| fig-width: 7
#| fig-height: 5
set.seed(353)
v <- round(rnorm(1000, mean = 36.8, sd = 0.35), 1)
histogram_with_pdf(v, binwidth = 0.1)
```

OK, that gives us more of an idea, but this picture is rather ragged. So let's try 10,000 observations; this is what I got:

```{r}
#| label: fig-temps-10k-1dp
#| fig-cap: "Body-temperature histogram: 10,000 readings to one decimal place"
#| fig-alt: "Bar histogram of 10,000 body-temperature readings rounded to one decimal place. The bell shape is now much smoother and the bars track the overlaid normal density closely from about 35.5°C to 38°C."
#| fig-width: 7
#| fig-height: 5
set.seed(353)
v <- round(rnorm(10000, mean = 36.8, sd = 0.35), 1)
histogram_with_pdf(v, binwidth = 0.1)
```
...
```{r}
#| label: fig-temps-10k-2dp
#| fig-cap: "Body-temperature histogram: 10,000 readings to two decimal places"
#| fig-alt: "Bar histogram of 10,000 body-temperature readings rounded to two decimal places. Hundreds of very narrow bars trace a noisier but unmistakable bell shape; the overlaid normal density runs through the middle of the noise."
#| fig-width: 7
#| fig-height: 5
set.seed(353)
v <- round(rnorm(10000, mean = 36.8, sd = 0.35), 2)
histogram_with_pdf(v, binwidth = 0.01)
```
...
```{r}
#| label: fig-temps-100k-2dp
#| fig-cap: "Body-temperature histogram: 100,000 readings to two decimal places"
#| fig-alt: "Bar histogram of 100,000 body-temperature readings rounded to two decimal places. The bell shape is markedly smoother than at 10,000 readings, with only a little jitter at the peak; the overlaid normal density is almost indistinguishable from the top of the bars."
#| fig-width: 7
#| fig-height: 5
set.seed(353)
v <- round(rnorm(100000, mean = 36.8, sd = 0.35), 2)
histogram_with_pdf(v, binwidth = 0.01)
```
...
```{r}
#| label: fig-temps-1m-2dp
#| fig-cap: "Body-temperature histogram: 1,000,000 readings to two decimal places"
#| fig-alt: "Bar histogram of 1,000,000 body-temperature readings rounded to two decimal places. The bell is now nearly perfectly smooth and the overlaid normal density sits squarely on top of the bar tops; only a faint wobble remains near the peak."
#| fig-width: 7
#| fig-height: 5
set.seed(353)
v <- round(rnorm(1000000, mean = 36.8, sd = 0.35), 2)
histogram_with_pdf(v, binwidth = 0.01)
```
...
```{r}
#| label: fig-temps-10m-2dp
#| fig-cap: "Body-temperature histogram: 10,000,000 readings to two decimal places"
#| fig-alt: "Bar histogram of 10,000,000 body-temperature readings rounded to two decimal places. The histogram is now visually indistinguishable from a smooth normal density; the overlaid blue curve runs exactly along the top of the bars across the whole range."
#| fig-width: 7
#| fig-height: 5
set.seed(353)
v <- round(rnorm(10000000, mean = 36.8, sd = 0.35), 2)
histogram_with_pdf(v, binwidth = 0.01)
```
...
```{r}
#| label: fig-area-percentages
#| fig-cap: "The three normal curves with the standard 'proportions of area thereunder' shown between successive multiples of σ"
#| fig-alt: "Three stacked panels — one each for σ = 1, σ = 2 and σ = 3 — each showing a normal curve centred on μ at 0 with the area under it divided into shaded bands and labelled with the standard percentages. In every panel the central inner ±σ band is shaded blue and labelled 34.1% on each side of μ; the next outward band from ±σ to ±2σ is shaded red and labelled 13.6% on each side; the band from ±2σ to ±3σ is shaded mid-grey and labelled 2.15% on each side; and the two extreme tails beyond ±3σ are shaded a darker red and labelled 0.135% via leader arrows. The panels share x and y scales, so the σ = 1 curve is tall and narrow, the σ = 2 curve is medium, and the σ = 3 curve is short and wide — yet the percentages in each panel are identical, illustrating that they apply irrespective of σ."
#| fig-width: 7
#| fig-height: 10
#| lightbox: true
# patchwork is already on the search path via R/setup.R → main-functions.R;
# the `/` composition operator below comes from that load.

# One panel per σ. Bands and labels are expressed in σ-units so each panel
# carries the same percentage labels (34.1 / 13.6 / 2.15 / 0.135) on its own
# curve — the whole point of the figure is that these percentages apply
# irrespective of σ.
area_percentages_panel <- function(sigma, strip_label) {
  xx   <- seq(-10, 10, length.out = 1001)
  dens <- data.frame(x = xx, y = dnorm(xx, sd = sigma))
  peak <- dnorm(0, sd = sigma)

  ribbon_band <- function(lo_z, hi_z, fill, alpha) {
    sub <- dens[dens$x >= lo_z * sigma & dens$x <= hi_z * sigma, ]
    geom_ribbon(data = sub, aes(x = .data$x, ymin = 0, ymax = .data$y),
                fill = fill, alpha = alpha, inherit.aes = FALSE)
  }

  # Label y-positions are expressed as a fraction of this panel's peak, so
  # they sit at the same *relative* height on every curve.
  y_at <- function(frac) frac * peak

  # Tail annotation x-positions, capped so they stay inside the shared
  # (-10, 10) xlim on the σ = 3 panel (where 3.7σ = 11.1 would otherwise
  # be clipped). For σ ≤ 2 the σ-relative positions fall well inside the
  # cap and the original geometry is preserved unchanged.
  tail_anchor_x <- min(3.15 * sigma, 9.0)
  tail_label_x  <- min(3.7  * sigma, 9.5)

  ggplot(dens, aes(x = .data$x, y = .data$y)) +
    ribbon_band(-1,  1, CONTROL_LIMIT_COLOUR, 0.25) +
    ribbon_band(-2, -1, CHART_LINE_COLOUR,    0.30) +
    ribbon_band( 1,  2, CHART_LINE_COLOUR,    0.30) +
    ribbon_band(-3, -2, CHART_GRID,           0.50) +
    ribbon_band( 2,  3, CHART_GRID,           0.50) +
    ribbon_band(-4, -3, CHART_LINE_COLOUR,    0.70) +
    ribbon_band( 3,  4, CHART_LINE_COLOUR,    0.70) +
    geom_line(colour = CHART_FG, linewidth = 0.6) +
    annotate("segment", x = 0, xend = 0, y = 0, yend = peak,
             colour = CHART_FG, linewidth = 0.3) +
    # Percentage labels in the body of the curve.
    annotate("text", x = -0.5 * sigma, y = y_at(0.45), label = "34.1%",
             colour = CHART_FG, size = 4) +
    annotate("text", x =  0.5 * sigma, y = y_at(0.45), label = "34.1%",
             colour = CHART_FG, size = 4) +
    annotate("text", x = -1.5 * sigma, y = y_at(0.18), label = "13.6%",
             colour = CHART_FG, size = 3.5) +
    annotate("text", x =  1.5 * sigma, y = y_at(0.18), label = "13.6%",
             colour = CHART_FG, size = 3.5) +
    annotate("text", x = -2.45 * sigma, y = y_at(0.045), label = "2.15%",
             colour = CHART_FG, size = 3.2) +
    annotate("text", x =  2.45 * sigma, y = y_at(0.045), label = "2.15%",
             colour = CHART_FG, size = 3.2) +
    # Tail leader arrows + 0.135% labels (x-positions computed above).
    annotate("segment", x = -tail_label_x, xend = -tail_anchor_x,
             y = y_at(0.075), yend = y_at(0.012),
             colour = CHART_FG, linewidth = 0.3,
             arrow = arrow(length = unit(0.12, "cm"))) +
    annotate("text", x = -tail_label_x, y = y_at(0.085), label = "0.135%",
             colour = CHART_FG, size = 3.2, hjust = 0.5, vjust = 0) +
    annotate("segment", x = tail_label_x, xend = tail_anchor_x,
             y = y_at(0.075), yend = y_at(0.012),
             colour = CHART_FG, linewidth = 0.3,
             arrow = arrow(length = unit(0.12, "cm"))) +
    ...
    # Shared y limits across panels (set to the σ = 1 peak, which is the
    # tallest of the three) so the height progression reads directly.
    scale_y_continuous(limits = c(0, dnorm(0) * 1.05), expand = c(0, 0)) +
    labs(subtitle = strip_label) +
    run_chart_theme(gridlines = "none") +
...
area_percentages_panel(1, "Smallish σ (σ = 1)") /
  area_percentages_panel(2, "Larger σ (σ = 2)") /
  area_percentages_panel(3, "Still larger σ (σ = 3)")
```
...
On [pages 54 and 55](04-part-d-crash-course.qmd#sec-optional-extras-page-54) I briefly describe the two most commonly-used methods of statistical inference, and the way in which these particular figures apply to them.

```{r}
#| label: fig-conf-95
#| fig-cap: "95% confidence region: the yellow area between μ − 1.96σ and μ + 1.96σ"
...
#| fig-height: 4
#| lightbox: true
conf_interval_plot(0.95, z = 1.96)
```

```{r}
#| label: fig-conf-99
#| fig-cap: "99% confidence region: the yellow area between μ − 2.58σ and μ + 2.58σ"
...
#| fig-cap: "Pdf of the uniform distribution"
...
#| fig-height: 3.5
#| lightbox: true
# Deterministic — closed-form uniform density evaluated on a fixed grid.
unif_a <- -sqrt(3); unif_b <- sqrt(3)
unif_pdf <- function(x) dunif(x, min = unif_a, max = unif_b)

x_pad   <- 0.5
xx      <- seq(unif_a - x_pad, unif_b + x_pad, length.out = 401)
df      <- data.frame(x = xx, y = unif_pdf(xx))
peak    <- 1 / (unif_b - unif_a)

ggplot(df, aes(x = .data$x, y = .data$y)) +
  annotate("segment", x = unif_a - x_pad, xend = unif_b + x_pad,
           y = 0, yend = 0,
           colour = CHART_FG, linewidth = 0.5) +
  annotate("segment", x = unif_a, xend = unif_a, y = 0, yend = peak,
           colour = CHART_FG, linewidth = 0.9) +
  annotate("segment", x = unif_b, xend = unif_b, y = 0, yend = peak,
           colour = CHART_FG, linewidth = 0.9) +
  annotate("segment", x = unif_a, xend = unif_b, y = peak, yend = peak,
           colour = CHART_FG, linewidth = 0.9) +
  scale_x_continuous(limits = c(unif_a - x_pad, unif_b + x_pad),
                     expand = c(0, 0)) +
  scale_y_continuous(limits = c(-peak * 0.18, peak * 1.25),
                     expand = c(0, 0)) +
  theme_void() +
...
```{r}
#| label: fig-uniform-n4
#| fig-cap: "Histogram of standardised $\\bar{X}$ from a uniform distribution, *n* = 4"
#| fig-alt: "Histogram of the standardised sample mean Z from 100,000 samples of size n = 4 drawn from a uniform distribution on [−√3, √3]. The histogram is now visually almost indistinguishable from the overlaid standard normal N(0, 1) density — symmetric, bell-shaped, centred on 0, with the curve sitting along the tops of the bars across the full range."
#| fig-width: 7
#| fig-height: 4.5
#| lightbox: true
set.seed(356)
...
```{r}
#| label: fig-uniform-n10
#| fig-cap: "Histogram of standardised $\\bar{X}$ from a uniform distribution, *n* = 10"
#| fig-alt: "Histogram of the standardised sample mean Z from 100,000 samples of size n = 10 drawn from a uniform distribution on [−√3, √3]. The histogram is indistinguishable by eye from the overlaid standard normal density: a clean, symmetric, centred bell with the blue N(0, 1) curve sitting exactly on top."
#| fig-width: 7
#| fig-height: 4.5
#| lightbox: true
set.seed(356)
...
```{r}
#| label: fig-tri-pdf
#| fig-cap: "Pdf of the triangular distribution"
...
#| fig-height: 3.5
#| lightbox: true
# Deterministic — closed-form triangular density evaluated on a fixed grid.
# Right-triangle PDF on [0, b] with mode at a = 0 and variance (b - a)^2 / 18.
# Setting variance = 1 gives b = 3*sqrt(2); the PDF reads
# f(x) = 2 * (b - x) / b^2 on [0, b]. Same parametrisation as Part E
# fig-part-e-pdfs.
tri_b <- 3 * sqrt(2)
tri_pdf <- function(x) {
  ifelse(x >= 0 & x <= tri_b, 2 * (tri_b - x) / tri_b^2, 0)
}

x_pad <- 0.4
xx    <- seq(-x_pad, tri_b + x_pad, length.out = 401)
df    <- data.frame(x = xx, y = tri_pdf(xx))
peak  <- 2 / tri_b

ggplot(df, aes(x = .data$x, y = .data$y)) +
  annotate("segment", x = -x_pad, xend = tri_b + x_pad,
           y = 0, yend = 0,
           colour = CHART_FG, linewidth = 0.5) +
  geom_line(colour = CHART_FG, linewidth = 0.9) +
  annotate("segment", x = 0, xend = 0, y = 0, yend = peak,
           colour = CHART_FG, linewidth = 0.9) +
  scale_x_continuous(limits = c(-x_pad, tri_b + x_pad), expand = c(0, 0)) +
  scale_y_continuous(limits = c(-peak * 0.18, peak * 1.25), expand = c(0, 0)) +
  theme_void() +
  theme(plot.background  = element_rect(fill = "transparent", colour = NA),
        panel.background = element_rect(fill = "transparent", colour = NA),
        plot.margin      = margin(8, 12, 8, 12))
...
```{r}
#| label: fig-tri-n2
#| fig-cap: "Histogram of standardised $\\bar{X}$ from a triangular distribution, *n* = 2"
...
#| fig-height: 4.5
#| lightbox: true
# Triangular (unsymmetric) on [0, 3*sqrt(2)] with mode at 0: σ = 1, μ = b/3 = √2.
# Inverse-CDF sampling: with F(x) = 1 - ((b - x) / b)^2 on [0, b], the inverse
# is x = b * (1 - sqrt(1 - u)).
tri_b   <- 3 * sqrt(2)
tri_mu  <- tri_b / 3   # √2
rtri    <- function(n) tri_b * (1 - sqrt(1 - runif(n)))
...
```{r}
#| label: fig-tri-n4
#| fig-cap: "Histogram of standardised $\\bar{X}$ from a triangular distribution, *n* = 4"
#| fig-alt: "Histogram of the standardised sample mean Z from 100,000 samples of size n = 4 drawn from the unsymmetric triangular distribution. The histogram is much closer to the overlaid standard normal density than at n = 2, though a slight right-skew remains — Neave's 'still slightly top-sided' observation — reflecting the lingering effect of the non-symmetric parent. The blue N(0, 1) curve sits almost on top of the bars across the bulk of the distribution."
#| fig-width: 7
#| fig-height: 4.5
#| lightbox: true
# Triangular constants redefined per-chunk — Quarto chunks run in independent
# scopes, so each chunk that uses tri_b / tri_mu / rtri redefines them here.
tri_b   <- 3 * sqrt(2)
tri_mu  <- tri_b / 3
rtri    <- function(n) tri_b * (1 - sqrt(1 - runif(n)))
...
```{r}
#| label: fig-tri-n10
#| fig-cap: "Histogram of standardised $\\bar{X}$ from a triangular distribution, *n* = 10"
#| fig-alt: "Histogram of the standardised sample mean Z from 100,000 samples of size n = 10 drawn from the unsymmetric triangular distribution. The bell shape is now almost perfectly symmetric and the overlaid blue N(0, 1) density runs along the tops of the bars across the full range — 'still not perfect, but it's almost there', as Neave's caption observes."
#| fig-width: 7
#| fig-height: 4.5
#| lightbox: true
# Triangular constants redefined per-chunk — Quarto chunks run in independent
...
```{r}
#| label: fig-exp-pdf
#| fig-cap: "Pdf of the exponential distribution"
...
#| fig-height: 3.5
#| lightbox: true
# Deterministic — closed-form exponential density evaluated on a fixed grid.
# Rate 1 exponential has mean = sd = 1, matching the σ = 1 parametrisation
# used throughout pages 52–54 (see fig-uniform-pdf and fig-tri-pdf above).
exp_rate <- 1
exp_pdf  <- function(x) dexp(x, rate = exp_rate)

x_pad <- 0.3
x_max <- 6
xx    <- seq(-x_pad, x_max, length.out = 401)
df    <- data.frame(x = xx, y = exp_pdf(xx))
peak  <- exp_rate

ggplot(df, aes(x = .data$x, y = .data$y)) +
  annotate("segment", x = -x_pad, xend = x_max,
           y = 0, yend = 0,
           colour = CHART_FG, linewidth = 0.5) +
  geom_line(colour = CHART_FG, linewidth = 0.9) +
  annotate("segment", x = 0, xend = 0, y = 0, yend = peak,
           colour = CHART_FG, linewidth = 0.9) +
  scale_x_continuous(limits = c(-x_pad, x_max), expand = c(0, 0)) +
  scale_y_continuous(limits = c(-peak * 0.18, peak * 1.25), expand = c(0, 0)) +
  theme_void() +
  theme(plot.background  = element_rect(fill = "transparent", colour = NA),
        panel.background = element_rect(fill = "transparent", colour = NA),
...
```{r}
#| label: fig-exp-n2
#| fig-cap: "Histogram of standardised $\\bar{X}$ from an exponential distribution, *n* = 2"
...
```{r}
#| label: fig-exp-n10
#| fig-cap: "Histogram of standardised $\\bar{X}$ from an exponential distribution, *n* = 10"
#| fig-alt: "Histogram of the standardised sample mean Z from 100,000 samples of size n = 10 drawn from an exponential distribution with rate 1. The histogram is much closer to symmetric than at n = 4 — the right-skew is now subtle — and the overlaid blue N(0, 1) density tracks the bar tops well across most of the range. Some residual top-sided asymmetry remains, reflecting the lingering effect of the highly non-symmetric parent."
#| fig-width: 7
#| fig-height: 4.5
#| lightbox: true
set.seed(357)
...
```{r}
#| label: fig-exp-n100
#| fig-cap: "Histogram of standardised $\\bar{X}$ from an exponential distribution, *n* = 100"
#| fig-alt: "Histogram of the standardised sample mean Z from 100,000 samples of size n = 100 drawn from an exponential distribution with rate 1. The histogram is now visually almost indistinguishable from the overlaid standard normal N(0, 1) density — symmetric, bell-shaped, centred on 0, with the blue curve sitting along the tops of the bars. Even the highly unsymmetric exponential parent has been tamed by the Central Limit Theorem at this sample size."
#| fig-width: 7
#| fig-height: 4.5
#| lightbox: true
set.seed(357)
...
is near enough to being standard normal, then, as we have seen on [page 50](04-part-d-crash-course.qmd#sec-optional-extras-page-50), there is about a 95% probability of *Z* lying between −1.96 and +1.96. A little algebra enables this range to be expressed as
...
I expect you will immediately appreciate (referring again to [page 50](04-part-d-crash-course.qmd#sec-optional-extras-page-50)) that a "99% confidence interval" can be obtained by replacing 1.96 by 2.58 throughout this discussion.
