...
pagetitle: "Optional Extras Part E: is there anything normal about control charts?"
description: "Two computer simulations disproving the claims that control-chart constants require normal data and that a 3σ signal has a 0.0027 probability."
execute:
  echo: false
---
...
## 1. Back to basics {#sec-optional-extras-page-57}
...
So, thinking back to the very [beginning of the crash-course (page 37)](04-part-d-crash-course.qmd#sec-optional-extras-page-37), let's get back to reality.
...
Full details of this simulation study and the results it produced are provided in [Section 4 (pages 68–70)](05-part-e-normality.qmd#sec-optional-extras-page-68).

## 3. "Control-chart constants depend on normally-distributed data, so unless your data are normally distributed your control chart isn't valid." {#sec-optional-extras-page-65}

[As observed on page 57](05-part-e-normality.qmd#sec-optional-extras-page-57), the familiar 2.66 does depend on the assumption that we have normally distributed data (in conjunction with Shewhart's 3σ-guidance). The dependence arises since the [conversion factors *h* tabulated on page 20](02-part-b-subgrouped.qmd#sec-optional-extras-page-20) depend on that assumption. With moving ranges effectively being subgroups of size two, the relevant value of *h* is 1.128 so that *MR̄* ÷ 1.128 becomes a "useful" estimator of σ under that normality assumption. The 2.66 then arises as the result of dividing 3 by 1.128 since this makes the resulting control limits consistent with Shewhart's 3σ-guidance. By "valid" in this context is the implication that the estimator is equal to σ "on the average" in the long term, making it a so-called "unbiased" estimator of σ; there is more discussion on this matter on [page 82 in the Technical Section](06-part-f-technical.qmd#sec-optional-extras-page-82). However, being correct on the average in the long term doesn't really say much about how close to σ we expect it to be in the short term, i.e. on the occasions when we actually use it! So my initial aim when writing this first of the two computer simulations was to find out how this estimator actually behaves in practice.

I therefore wrote a program to generate a large number of series of data from a normal distribution having σ = 1, and in each case calculate *MR̄* ÷ 1.128. I chose to use baselines (series-lengths) of 12 and to generate 10 million such series. The program then drew a histogram of those 10 million estimates. That histogram is shown at the [top left of page 67](05-part-e-normality.qmd#sec-optional-extras-page-67) and, as you can see, the values of the estimator varied from below 0.5 to above 2.0. That may well strike you as rather wider variation around the true value of σ = 1 than you might like. However, to substantially reduce that variation would require a much longer baseline. There are many arguments as to why that's undesirable. Some reasons are discussed in detail in [Section 1 of Part F (pages 71–74)](06-part-f-technical.qmd#sec-optional-extras-page-71). A further reason which is not raised there is that, unless data are arriving thick and fast (impossible with many processes), there will a considerable delay before the control chart can begin to be used. As yet a further objection, there's a law of decreasing returns in operation here: for example, to halve the width of this histogram would require *quadrupling* the baseline length to 48—wholly unsatisfactory for the numerous reasons just referred to. However, that whole issue is another matter. We are simply concerned at the moment with the Mathematical Statistician's claim that, because that 2.66 figure depends on the assumption of a normal distribution, this method is "not valid" if the data do not fit that assumption.

There's an obvious way to examine this (without raising the tricky matter of exact stability not being feasible!). And that is to modify the program so as to generate data from distributions other than the normal distribution and take a look at those resulting histograms. I used the same three distributions as in the [simulation study on the Central Limit Theorem (pages 51–54)](04-part-d-crash-course.qmd#sec-optional-extras-page-51): the uniform distribution, the unsymmetric triangular distribution and the exponential distribution, each being in a version with σ = 1 for ease of comparison with the initial normal version. You've seen [pictures of these distributions on pages 52–53 in Part D](04-part-d-crash-course.qmd#sec-optional-extras-page-52) but, for your convenience, they are illustrated again on the next page along with the normal distribution for comparison.

```{r}
#| label: fig-part-e-pdfs
#| fig-cap: "The four probability distributions used in the simulation: normal, uniform, triangular (unsymmetric) and exponential — each with σ = 1."
#| fig-alt: "Four small probability-density plots arranged in a two-by-two grid, each rendered with a black ink curve over a thin baseline. Top-left: the bell-shaped normal density centred on zero. Top-right: the flat rectangular uniform density. Bottom-left: a right-triangle triangular density that peaks on the left and falls to zero on the right. Bottom-right: the exponential density, with a tall left edge that decays sharply to the right. Each panel auto-scales to its own peak so the four shapes are directly recognisable — heights are not comparable across panels."
#| fig-width: 8
#| fig-height: 6
#| lightbox: true
# patchwork is on the search path via R/setup.R → main-functions.R. The chunk
# is fully deterministic — all four panels use closed-form densities and a
# fixed grid, so no seed is needed.

# All four PDFs are σ = 1 versions of their family. Each panel auto-scales to
# its own peak — Neave's printed page 66 layout — so the comparison emphasises
# *shape* rather than density-value.
#
# Triangular (unsymmetric, right-triangle decreasing from left peak): for a
# triangular density on [a, b] with mode at a, the variance is (b - a)^2 / 18.
# Setting variance = 1 gives b - a = 3*sqrt(2). With a = 0 and b = 3*sqrt(2)
# the PDF reads f(x) = 2*(b - x) / (b - a)^2 on [a, b].
tri_b <- 3 * sqrt(2)
tri_pdf <- function(x) {
  ifelse(x >= 0 & x <= tri_b, 2 * (tri_b - x) / tri_b^2, 0)
}

# Uniform with σ = 1: half-width sqrt(3) so variance = (2*sqrt(3))^2 / 12 = 1.
unif_a <- -sqrt(3); unif_b <- sqrt(3)

# Each panel is a tiny stand-alone plot: thin black curve, thin black baseline,
# corner label, no axis ink. Neave's printed figure has no axes at all — only
# the curve, a baseline, and a corner caption — so we mirror that here.
mini_pdf_panel <- function(fun, xlim, label,
                           label_corner = c("tl", "tr", "bl", "br"),
                           n = 401) {
  ...
  # Label position: hugged to the chosen corner with a small inset.
  x_at <- switch(label_corner,
                 tl = xlim[1], bl = xlim[1],
                 tr = xlim[2], br = xlim[2])
  y_at <- switch(label_corner,
                 tl = ymax * 1.08, tr = ymax * 1.08,
                 bl = -ymax * 0.05, br = -ymax * 0.05)
  hjust <- if (label_corner %in% c("tl", "bl")) 0 else 1
  vjust <- if (label_corner %in% c("tl", "tr")) 0 else 1
  ...
  ggplot(df, aes(x = .data$x, y = .data$y)) +
    ...
    annotate("text", x = x_at, y = y_at, label = label,
             hjust = hjust, vjust = vjust,
             colour = CHART_FG, size = 4.5) +
    scale_x_continuous(limits = xlim, expand = c(0, 0)) +
    scale_y_continuous(
      limits = c(-ymax * 0.18, ymax * 1.20),
      expand = c(0, 0)
    ) +
    theme_void() +
...
p_normal <- mini_pdf_panel(
  function(x) dnorm(x, mean = 0, sd = 1),
  xlim = c(-4, 4), label = "NORMAL", label_corner = "tl"
)
p_uniform <- mini_pdf_panel(
  function(x) dunif(x, min = unif_a, max = unif_b),
  xlim = c(unif_a - 0.3, unif_b + 0.3), label = "UNIFORM",
  label_corner = "tr"
)
p_triangular <- mini_pdf_panel(
  tri_pdf,
  xlim = c(-0.3, tri_b + 0.3), label = "TRIANGULAR",
  label_corner = "bl"
)
p_exponential <- mini_pdf_panel(
  function(x) dexp(x, rate = 1),
  xlim = c(-0.2, 6), label = "EXPONENTIAL",
  label_corner = "br"
)

(p_normal | p_uniform) / (p_triangular | p_exponential)
```
...
The resulting histograms are shown on [page 67](05-part-e-normality.qmd#sec-optional-extras-page-67). Now, presumably if there *were* any truth in the notion that normality is *necessary* for the standard method of constructing control charts for one-at-a-time data to be useful, the top left histogram for the normal distribution should stand out as representing "good" behaviour while the others should be clearly "bad" in comparison. But, to put it mildly, such evidence is not easy to see! Despite its lack of symmetry, the triangular case appears to be virtually identical to the normal case while the uniform case is just a bit superior to the normal case! The exponential distribution's histogram is a little wider but, to be honest and seeing how incredibly different that shape of distribution is from the normal distribution, I was pleasantly surprised to see how *similar* to the others its histogram turned out to be. The overall conclusion from this simulation exercise surely has to be that the claim of data needing to be normally distributed in order to use the 2.66 *MR̄* method for constructing control charts lacks credibility.

[]{#sec-optional-extras-page-67}

```{r}
#| label: fig-part-e-xbar-histograms
#| fig-cap: "Histograms of the estimator *MR̄* ÷ 1.128 obtained from 200,000 simulated baselines of length 12 under each of the four distributions. Axes run from 0.0 to 2.5 in every panel, so the panels are directly comparable."
#| fig-alt: "Four histograms of the estimator MR-bar divided by 1.128 from simulated baselines of length 12, one for each of the normal, uniform, triangular and exponential distributions. Each histogram shares an x axis running from 0.0 to 2.5 with a tick at every 0.5. The normal, uniform and triangular histograms are visually almost indistinguishable — each is a broadly symmetric bell-shaped pile centred near 1.0 spanning roughly 0.5 to 1.5. The exponential histogram is a little wider and pulled slightly to the right, with its peak near 0.8 and a longer right tail running out toward 2.0 — but it is still strikingly similar in overall shape and spread to the other three."
#| fig-width: 8
#| fig-height: 6
#| lightbox: true
# patchwork is on the search path via R/setup.R → main-functions.R.
#
# For each distribution we draw N = 200,000 baselines of length 12, compute
# the mean moving range MR-bar = mean(|diff(x)|) for each baseline, then
# divide by Shewhart's d2 = 1.128 to obtain the estimator. 200,000 is
# comfortably enough to make the "strikingly similar" visual claim of
# Neave's page 67 obvious; Neave used 10,000,000, which is overkill at the
# resolution of a printed page and would render perceptibly slowly here.
set.seed(358)
...
# Triangular (unsymmetric, σ = 1) — same parametrisation as page 66 above.
# Inverse-CDF sampling: with F(x) = 1 - ((b - x) / b)^2 on [0, b], the
# inverse is x = b * (1 - sqrt(1 - u)). For σ = 1 we use b = 3*sqrt(2).
tri_b <- 3 * sqrt(2)
rtri  <- function(n) tri_b * (1 - sqrt(1 - runif(n)))

# Uniform with σ = 1: half-width sqrt(3). Exponential with σ = 1: rate = 1.
unif_a <- -sqrt(3); unif_b <- sqrt(3)

mr_bar_estimator <- function(rgen, n_baselines, n_per_baseline) {
  total <- n_baselines * n_per_baseline
  draws <- matrix(rgen(total), nrow = n_baselines, ncol = n_per_baseline)
  rowMeans(abs(draws[, -1] - draws[, -n_per_baseline])) / d2
}

normal_est      <- mr_bar_estimator(
  function(n) rnorm(n, mean = 0, sd = 1),
  n_baselines, n_per_baseline
)
uniform_est     <- mr_bar_estimator(
  function(n) runif(n, min = unif_a, max = unif_b),
  n_baselines, n_per_baseline
)
triangular_est  <- mr_bar_estimator(rtri, n_baselines, n_per_baseline)
exponential_est <- mr_bar_estimator(
  function(n) rexp(n, rate = 1),
  n_baselines, n_per_baseline
)

est_panel <- function(values, label) {
  ggplot(data.frame(v = values), aes(x = .data$v)) +
    geom_histogram(
      binwidth  = 0.025,
      boundary  = 0,
      closed    = "left",
    ...
    theme(
      ...
      plot.subtitle = element_text(colour = CHART_FG, size = 12,
                                   hjust = 0, face = "plain")
    )
}

p_normal_hist      <- est_panel(normal_est,      "NORMAL")
p_uniform_hist     <- est_panel(uniform_est,     "UNIFORM")
p_triangular_hist  <- est_panel(triangular_est,  "TRIANGULAR")
p_exponential_hist <- est_panel(exponential_est, "EXPONENTIAL")

(p_normal_hist | p_uniform_hist) /
  (p_triangular_hist | p_exponential_hist)
```

## 4. "If your data are normally distributed then the probability of a false signal is 0.0027" {#sec-optional-extras-page-68}

There was a fairly full introduction to this simulation study on [pages 63–64](05-part-e-normality.qmd#sec-optional-extras-page-63) and so I can now move straight into the details of the study.
...
```{r}
#| label: fig-part-e-12-subgroups
#| fig-cap: "Histograms of the simulated probability of a false signal on an *X̄*-chart constructed from 12 subgroups of size 2, 4 and 6 respectively. The target \"textbook\" value 0.0027 is marked in red on each axis, which runs from 0 to 0.020."
#| fig-alt: "Three histograms stacked vertically showing the simulated distribution of the probability of a false signal on an X-bar chart when control limits are computed from 12 subgroups, for subgroup sizes 2, 4 and 6. In each case the 0.0027 reference value is marked near the left of the axis, far inside a long and heavily spread histogram that extends well beyond 0.02."
#| fig-width: 8
#| fig-height: 9
#| lightbox: true
# patchwork is on the search path via R/setup.R → main-functions.R.
#
# For each subgroup size n ∈ {2, 4, 6} we draw N = 200,000 replications of
# 12 subgroups from N(0, 1), build the standard X-bar control limits
# (grand mean ± A2 · R-bar), and then compute the probability that a future
# subgroup mean N(0, 1/√n) falls outside those limits. Neave used one
# million; 200,000 is enough to make the page-69 visual claim — that the
# histograms are very spread out and that 0.0027 sits well to the *left*
# of the bulk of the distribution — read obviously, while keeping per-render
# render time tractable.
set.seed(359)

n_replications_p69 <- 200000L
m_subgroups_p69    <- 12L
x_max_p69          <- 0.020

probs_n2_p69 <- xbar_false_signal_probs(
  n = 2L, m_subgroups = m_subgroups_p69,
  n_replications = n_replications_p69
)
probs_n4_p69 <- xbar_false_signal_probs(
  n = 4L, m_subgroups = m_subgroups_p69,
  n_replications = n_replications_p69
)
probs_n6_p69 <- xbar_false_signal_probs(
  n = 6L, m_subgroups = m_subgroups_p69,
  n_replications = n_replications_p69
)

p_n2_p69 <- xbar_false_signal_panel(
  probs_n2_p69, "12 subgroups of size 2", x_max = x_max_p69
)
p_n4_p69 <- xbar_false_signal_panel(
  probs_n4_p69, "12 subgroups of size 4", x_max = x_max_p69
)
p_n6_p69 <- xbar_false_signal_panel(
  probs_n6_p69, "12 subgroups of size 6", x_max = x_max_p69
)

p_n2_p69 / p_n4_p69 / p_n6_p69
```
...
After seeing these histograms, I could imagine some statisticians complaining that to use only 12 subgroups for computing the limits was insufficient. I therefore repeated the whole simulation using 40 subgroups instead. 40 subgroups is a far larger number than most people use in practice. Those histograms are shown on [page 70](05-part-e-normality.qmd#sec-optional-extras-page-70).

[]{#sec-optional-extras-page-70}

```{r}
#| label: fig-part-e-40-subgroups
#| fig-cap: "Histograms of the simulated probability of a false signal on an *X̄*-chart constructed from 40 subgroups of size 2, 4 and 6 respectively. The 0.0027 reference value is again marked in red on each axis, which runs from 0 to 0.025."
...
# patchwork is on the search path via R/setup.R → main-functions.R.
#
# Same simulation engine as fig-part-e-12-subgroups, just with 40 subgroups
# instead of 12. Neave's page 70 uses x ∈ [0, 0.025] (a slightly wider axis
# than page 69) so that the long-but-thinner tails for n = 4 and n = 6
# still fit on the page; we mirror that here.
set.seed(360)

n_replications_p70 <- 200000L
m_subgroups_p70    <- 40L
x_max_p70          <- 0.025

probs_n2_p70 <- xbar_false_signal_probs(
  n = 2L, m_subgroups = m_subgroups_p70,
  n_replications = n_replications_p70
)
probs_n4_p70 <- xbar_false_signal_probs(
  n = 4L, m_subgroups = m_subgroups_p70,
  n_replications = n_replications_p70
)
probs_n6_p70 <- xbar_false_signal_probs(
  n = 6L, m_subgroups = m_subgroups_p70,
  n_replications = n_replications_p70
)

p_n2_p70 <- xbar_false_signal_panel(
  probs_n2_p70, "40 subgroups of size 2", x_max = x_max_p70
)
p_n4_p70 <- xbar_false_signal_panel(
  probs_n4_p70, "40 subgroups of size 4", x_max = x_max_p70
)
p_n6_p70 <- xbar_false_signal_panel(
  probs_n6_p70, "40 subgroups of size 6", x_max = x_max_p70
)

p_n2_p70 / p_n4_p70 / p_n6_p70
```

At least, with these latter histograms, one can see some evidence of the one thing we know about the values of the probability of a false signal. We can actually see that it is indeed feasible for the probability of a false signal to have a (very) long-term average of around 0.0027—admittedly not so with subgroups of size 2 but looking not unlikely with subgroups of size 4 or 6. However, this is already using far more data than is at all usual in practice. The disadvantages of using long baselines with subgrouped data are very much the same as with one-at-a-time data: recall that the latter are the subject of the [first part of the Technical Section which follows on page 71](06-part-f-technical.qmd#sec-optional-extras-page-71).
