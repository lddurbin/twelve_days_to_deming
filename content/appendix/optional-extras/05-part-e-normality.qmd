...
tri_b <- 3 * sqrt(2)
rtri  <- function(n) tri_b * (1 - sqrt(1 - stats::runif(n)))
...
mr_bar_estimator <- function(rgen, n_baselines, n_per_baseline) {
  ...
  mr_bar_vec <- rowMeans(abs(t(apply(draws, 1, diff))))
  mr_bar_vec / d2
}

normal_est      <- mr_bar_estimator(
  function(n) stats::rnorm(n, mean = 0, sd = 1),
  n_baselines, n_per_baseline
)
uniform_est     <- mr_bar_estimator(
  function(n) stats::runif(n, min = unif_a, max = unif_b),
  n_baselines, n_per_baseline
)
