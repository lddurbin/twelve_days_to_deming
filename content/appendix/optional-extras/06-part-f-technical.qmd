...
pagetitle: "Optional Extras Part F: why control-chart constants are what they are"
description: "Simulation evidence for choosing a baseline length, proofs behind the control-chart constants, and tables of the binomial and normal distributions."
execute:
  echo: false
---
...
## 3. Proofs and uses of expected values {#sec-optional-extras-page-79}
...
If it is a while since you read that material on the simple binomial distributions at the [beginning of Part D](04-part-d-crash-course.qmd#sec-optional-extras-page-37) then I suggest, in order to put yourself back in the picture, you skim through those [first few pages (from page 37)](04-part-d-crash-course.qmd#sec-optional-extras-page-37) now before continuing with this more general treatment.
...
That is, the first trial produced a Success, the second and third trials produced Failures, and so on. Seeing that these are *independent* trials with fixed probabilities of Success and Failure, we can use the [simple multiplication rule (page 77)](06-part-f-technical.qmd#sec-optional-extras-page-77) many times over to obtain the probability:
...
[]{#sec-optional-extras-page-76}Hooray! Now, that's all very well, but that will still involve a pretty unpleasant amount of arithmetic to actually get numerical values for all those probabilities! Also, the prospect of computing the mean and variance of the distribution using the formulae

$$\mu = \sum x \cdot \mathrm{Prob}(X = x) \quad \text{and} \quad \sigma^2 = \sum (x - \mu)^2 \cdot \mathrm{Prob}(X = x)$$

as shown at the [top of page 76](06-part-f-technical.qmd#sec-optional-extras-page-76) is also not particularly appealing! Let's deal with this latter issue first.

Fortunately, we can bypass those formulae by taking advantage of the additivity properties for both means and variances proved in [Section 3 (pages 79 and 80)](06-part-f-technical.qmd#sec-optional-extras-page-79). $X$, the number of successes, can of course be considered as
...
There's no difficulty in calculating the mean and variance of each of those! We have $\mathrm{Prob}(X_1 = 0) = q$ and $\mathrm{Prob}(X_1 = 1) = p$, and so clearly we have $\mathrm{E}[X_1] = p$, as is also true of $\mathrm{E}[X_2], \mathrm{E}[X_3], \ldots, \mathrm{E}[X_n]$. Thus $\mu = \mathrm{E}[X] = np$. The variance is almost as easy. Recall the useful result [(V1) on page 79](06-part-f-technical.qmd#sec-optional-extras-page-79): $\sigma^2 = \mathrm{E}[X^2] - \mu^2$, with both possible values of $X_1^2$ being fortunately *equal* to $X_1$! That being the case, $\mathrm{E}[X_1^2] = \mathrm{E}[X_1] = p$. So this gives us the variance of $X_1$ as $\mathrm{E}[X_1^2] - \{\mathrm{E}[X_1]\}^2 = p - p^2 = p(1-p) = pq$. The same will be true of $X_2, \ldots, X_n$. And so finally we can use the additivity property for variances (see [(V3) on page 80](06-part-f-technical.qmd#sec-optional-extras-page-80)) to obtain the variance of $X$, i.e. $\sigma^2$, as $npq$. Those important results are worth displaying:
...
[]{#sec-optional-extras-page-89}Finally, since the tables only go up to $n = 20$, how can we find probabilities for larger values of $n$ without needing to do a lot of arithmetic? For this purpose, and recalling the Central Limit Theorem, it is often possible to use tables of the normal distribution to obtain good approximations to binomial probabilities. So I'll return to that matter in the [bottom half of page 91](06-part-f-technical.qmd#sec-optional-extras-page-91) after now concluding the tale of widely-available tables of the normal distribution.
