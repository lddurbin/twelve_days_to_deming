...
```{ojs}
viewof counts = Inputs.text({
  label: "Enter counts (space-separated):",
  value: ""
})

// Parse the counts
countsArray = counts.split(" ").filter(x => x.trim() !== "").map(Number)
xArray = Array.from({length: countsArray.length}, (_, i) => i + 1)

// Calculate mean and standard deviation (only if we have data)
mean = countsArray.length > 0 ? countsArray.reduce((a, b) => a + b, 0) / countsArray.length : 0
std = countsArray.length > 1 ? Math.sqrt(countsArray.map(x => (x - mean) ** 2).reduce((a, b) => a + b, 0) / (countsArray.length - 1)) : 0

// User sets their own control limits
viewof ucl = Inputs.number({
  label: "Upper Control Limit (UCL):", 
  value: 0, 
  step: 0.01,
  min: 0
})
viewof lcl = Inputs.number({
  label: "Lower Control Limit (LCL):", 
  value: 0, 
  step: 0.01,
  min: 0
})
viewof meanLine = Inputs.number({
  label: "Mean Line:", 
  value: 0, 
  step: 0.01,
  min: 0
...
countsArray.length > 0 ? Plot.plot({
  marks: [
    Plot.line(xArray.map((x, i) => ({x, y: countsArray[i]})), {x: "x", y: "y", stroke: "steelblue", strokeWidth: 2}),
    Plot.dot(xArray.map((x, i) => ({x, y: countsArray[i]})), {x: "x", y: "y", fill: "steelblue", r: 3}),
    // Add mean line
    Plot.ruleY([meanLine], {stroke: "green", strokeDasharray: "4 2", strokeWidth: 2, label: "Mean"}),
    // Add UCL and LCL lines
    Plot.ruleY([ucl], {stroke: "red", strokeDasharray: "2 2", strokeWidth: 2, label: "UCL"}),
    Plot.ruleY([lcl], {stroke: "red", strokeDasharray: "2 2", strokeWidth: 2, label: "LCL"})
  ],
  y: {
    label: "Red Beads Count", 
    domain: [
      Math.min(0, lcl, Math.min(...countsArray)) * 0.9, 
      Math.max(...countsArray, ucl) * 1.1
    ]
  },
  x: {label: "Time", domain: [1, countsArray.length]},
  width: 800,
  height: 400
}) : null
```
