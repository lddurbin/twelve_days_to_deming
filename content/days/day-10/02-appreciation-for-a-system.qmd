...
pagetitle: "Day 10: Part A — Appreciation for a system"
description: "Deming's definition of a system, the aim of 'everybody to gain', and why a business needs even more teamwork than an orchestra or a bowling team."
execute:
  echo: false
---

## PART A: APPRECIATION FOR A SYSTEM {#sec-page3}

### Step 1: Browsing session

Relevant reading:

> Prelude A: "Four Preludes and a Coda": [pages 1–6, starting "Understanding a System"](../../appendix/contributions-balaji-reddie/01-prelude-a-system.qmd#sec-page1). (From now on I'll simply refer to "Preludes".)
>
> *DemDim*: pages 264–270.
>
> Today's material: [pages 3–14, starting with Part A below](02-appreciation-for-a-system.qmd#sec-page3) [*WB* 154–165].
...
viewof note_10_components = persist(
  "content/days/day-10/02-appreciation-for-a-system.qmd#note_10_components",
  Inputs.textarea({label: "Your notes on the system's components and management's role", placeholder: "Your notes on this item.", rows: 4})
)
```
...
```{ojs}
viewof note_10_aim = persist(
  "content/days/day-10/02-appreciation-for-a-system.qmd#note_10_aim",
  Inputs.textarea({label: "Your notes on 'the aim of the system' (everybody to gain)", placeholder: "Your notes on this item.", rows: 4})
)
```
...
```{ojs}
viewof note_10_orchestra_business = persist(
  "content/days/day-10/02-appreciation-for-a-system.qmd#note_10_orchestra_business",
  Inputs.textarea({label: "Your notes on the orchestra example and a business as a network", placeholder: "Your notes on this item.", rows: 4})
)
```
...
```{ojs}
viewof note_10_schools_performance = persist(
  "content/days/day-10/02-appreciation-for-a-system.qmd#note_10_schools_performance",
  Inputs.textarea({label: "Your notes on the system of schools and judging a component's performance", placeholder: "Your notes on this item.", rows: 4})
)
```
...
```{ojs}
viewof note_10_poor_management = persist(
  "content/days/day-10/02-appreciation-for-a-system.qmd#note_10_poor_management",
  Inputs.textarea({label: "Your notes on the examples of poor management (lowest price, travel costs, etc.)", placeholder: "Your notes on this item.", rows: 4})
)
```
...
```{ojs}
viewof note_10_winlose = persist(
  "content/days/day-10/02-appreciation-for-a-system.qmd#note_10_winlose",
  Inputs.textarea({label: "Your notes on Win–Lose structures and optimisation as the basis for negotiation", placeholder: "Your notes on this item.", rows: 4})
)
```

<div class="deming_quote">
Some examples follow of suboptimisation in the management of people, causing losses unknown and unknowable: [*You might find it useful to refer back to the "Optimise or maximise?" section beginning on "Preludes" [page 5](../../appendix/contributions-balaji-reddie/01-prelude-a-system.qmd#sec-page5). Deming is simply pointing out here that these examples all indicate the existence of suboptimised rather than optimised systems. Thus they are all indicating that there is a better way.*]
...
- Work standards for production; quotas for sales; quotas for accidents and breakdowns; [*If necessary, refer back to [Day 5 page 27](../day-05/03-the-deadly-diseases.qmd#sec-page27) to remind yourself of more about what Deming meant by "work standards".*]
...
<div class="deming_quote">
Fortunately, precise optimisation is not necessary. One need only come close to optimisation. As a matter of fact, the precise optimum would be difficult to define. The loss function will apply [*as emphasised by Taguchi*]. The loss function will be at the bottom (minimum loss) a parabola. Curve and horizontal tangent are for practical purposes coincident over a short range. [*You could look back at [Day 7 page 22](../day-07/05-thirteenth-obstacle.qmd#sec-page22) to remind yourself about the shape of the Taguchi loss function. The important sentence in this paragraph is the final one which now follows:*] One may move away a short distance along the curve in either direction from the optimum, but rise only an imperceptible distance.
</div>
...
```{ojs}
viewof note_10_suboptimisation_taguchi = persist(
  "content/days/day-10/02-appreciation-for-a-system.qmd#note_10_suboptimisation_taguchi",
  Inputs.textarea({label: "Your notes on examples of suboptimisation and the relevance of the Taguchi loss function", placeholder: "Your notes on this item.", rows: 4})
)
```
...
```{ojs}
viewof note_10_demdim_parta = persist(
  "content/days/day-10/02-appreciation-for-a-system.qmd#note_10_demdim_parta",
  Inputs.textarea({label: "Additional notes after reading DemDim pages 264–270 on Appreciation for a System", placeholder: "Additional notes from DemDim pages 264–270.", rows: 6})
)
```
