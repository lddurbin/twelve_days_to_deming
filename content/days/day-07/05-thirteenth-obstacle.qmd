...
pagetitle: "Day 7: The 13th obstacle — meeting specifications isn't enough"
description: "A hands-on temperature exercise introduces the Taguchi loss function, showing why conformance to specification is a crude, illogical measure of quality."
session_minutes: 25
execute:
  echo: false
---
...
To confirm, this is just the same thinking as we have with targets except that "OK" now corresponds to being within the range defined by two numbers whereas, with targets, "OK" corresponds to reaching or going beyond the single defining number. The Gallery Furniture salesman would get his 10% commission if he sold $7,000 of furniture or more ([Day 6 page 8](../day-06/02-gallery-furniture.qmd#sec-page8)); Dr Deming's student, Christine, would have been "OK" if she made 25 or more calls per hour ([Day 6 page 22](../day-06/04-more-true-stories.qmd#sec-page22)); the performance of the police was "OK" if they reached the scene of the crime in 10 minutes or less (same page).
...
```{r specification-limits, fig.width=8, fig.height=1.6, fig.align="center", fig.cap="Specification limits: LSL and USL with four results — *b* and *a* near the lower limit, *c* and *d* near the upper. By conformance-to-specification, *a* and *c* pass while *b* and *d* fail — yet there is essentially no practical difference between *a* and *b*, nor between *c* and *d*."}
...
viewof activity_7e_1 = persist(
  "content/days/day-07/05-thirteenth-obstacle.qmd#activity_7e_1",
  Inputs.text({label: "Step 1: Your most comfortable, 'ideal' working temperature", placeholder: "e.g. 21°C", width: 200})
)
```
...
```{ojs}
viewof activity_7e_2 = persist(
  "content/days/day-07/05-thirteenth-obstacle.qmd#activity_7e_2",
  Inputs.text({label: "Step 2: A fairly wide temperature scale around your ideal value (seven temperatures)", placeholder: "Write your seven temperatures here, e.g. 6°C, 11°C, 16°C, 21°C, 26°C, 31°C, 36°C", width: 600})
)
```
...
```{ojs}
viewof activity_7e_3 = persist(
  "content/days/day-07/05-thirteenth-obstacle.qmd#activity_7e_3",
  Inputs.textarea({label: "Step 3: Approximate percentages of your work quality at each of the seven temperatures", placeholder: "Write your temperatures and percentages here, e.g.\n6°C → 5%\n11°C → 50%\n16°C → 85%\n21°C → 100%\n26°C → 90%\n31°C → 55%\n36°C → 15%", rows: 8})
)
```
...
```{ojs}
viewof activity_7e_4 = persist(
  "content/days/day-07/05-thirteenth-obstacle.qmd#activity_7e_4",
  Inputs.textarea({label: "Step 4: Percentage loss in your work at each temperature (100% minus your Step 3 values)", placeholder: "Write your temperatures and percentage losses here, e.g.\n6°C → 95%\n11°C → 50%\n16°C → 15%\n21°C → 0%\n26°C → 10%\n31°C → 45%\n36°C → 85%", rows: 8})
)
```
...
```{r taguchi-loss-abstract, fig.width=6, fig.height=3.2, fig.align="center", fig.cap="The abstract Taguchi loss function: a symmetric parabola, bottoming out at the *nominal* value and rising symmetrically away from it."}
...
```{r taguchi-loss-personal, fig.width=7, fig.height=5, fig.align="center", fig.cap="Neave's plotted personal graph: seven X markers at his temperature/loss readings (95% at 6°C, 50% at 11°C, 15% at 16°C, 0% at 21°C, 10% at 26°C, 45% at 31°C, 85% at 36°C) joined by a smooth curve — recognisably the same U-shape, but slightly asymmetric (losses rise faster on the cold side than on the hot)."}
