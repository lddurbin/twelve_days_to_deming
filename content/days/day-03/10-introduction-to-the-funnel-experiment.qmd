...
pagetitle: "Day 3: Introducing the Funnel Experiment"
description: "Introducing Lloyd Nelson's Funnel Experiment and Neave's simplified one-dimensional version, built from a numbered track, a funnel, a marble and dice."
execute:
  echo: false
---
[]{#sec-page35}
...
```{r funnel-track-basic, fig.width=9, fig.height=1.5, fig.align="center", fig.cap="A horizontal track of 21 numbered squares, running from 20 on the left to 40 on the right. The middle square (30) is overlaid with a bullseye marker — the target-point for the silly game."}
funnel_track_plot()
```
...
```{r funnel-track-with-icons, fig.width=9, fig.height=1.5, fig.align="center", fig.cap="The same 20–40 track, now with two markers above it: a blue downward triangle above square 31 (the funnel's position) and a small yellow circle above square 33 (where the marble came to rest)."}
funnel_track_plot(funnel_pos = 31, marble_pos = 33)
```
...
In our timing schedule we are now coming up to the lunchtime break. If you have not already done so, you will now need to prepare your own track like the one illustrated in order for you to be able to carry out our version of the Funnel Experiment, ready to use when we resume this afternoon. On [page 59, below](10-introduction-to-the-funnel-experiment.qmd#sec-page59) you will find a larger copy of the track in two parts, one part going from 20 to 30 and the other from 30 to 40. Preferably (but only if convenient) I suggest that you copy or paste these (either electronically or using glue!) onto some card. Then join them together into a single strip as shown above by attaching the two 30s one on top of the other. I've also included on [page 59, below](10-introduction-to-the-funnel-experiment.qmd#sec-page59) larger images of both the funnel and the marble: you could similarly put the image of each that you choose onto small pieces of card to use in the experiment. Alternatively, you could e.g. use a large silver coin to represent the funnel and a small copper/bronze coin to represent the marble.

Finally, as previously warned (on [Day 2 page 44](../day-02/09-postscript.qmd#sec-page44)), you will also need two standard dice (and a shaker if you so wish!).
...
```{r funnel-dice-sequences, fig.width=14, fig.height=4, fig.align="center", fig.cap="Two pre-recorded sequences of dice-throw outcomes — Seq. A and Seq. B — provided as a fallback for any reader without physical dice. Each cell shows a pair of dice values (one for each of two dice). Both sequences begin at Stage 6: the first five stages are supplied separately for demonstration."}
...
[]{#sec-page59}
![Print-ready track for cutting out, together with the funnel and marble images](/assets/images/day-03/funnel-track-colourful.png){fig-align="center" width="60%" .lightbox}
...
[]{#sec-page61}
![Print-ready extension strips for the track](/assets/images/day-03/funnel-track-extensions.png){fig-align="center" width="55%" .lightbox}
