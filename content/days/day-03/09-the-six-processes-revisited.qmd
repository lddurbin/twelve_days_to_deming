...
pagetitle: "Day 3: The Six Processes revisited — spotting special causes"
description: "Extending the Six Processes charts to 48 points each, tracing exactly where and how special causes show up, from a beta-blocker to a financial crisis."
session_minutes: 25
session_minutes_stats0: 20
execute:
  echo: false
---
[]{#sec-page30}

On [pages 19–23, starting in the Six Processes section above](06-six-processes.qmd#sec-page19) I described and discussed how our study of the six processes can help us diagnose whether a process is or is not in statistical control. But it can do more than that. If it looks as if the process is being affected by special causes then the control chart can often give us useful guidance about when (and thus often where) to look for them.
...
```{r chart-a3-individual, fig.width=9, fig.height=3.5, fig.align="center", fig.cap="Chart A3: dice totals across all 48 points (A1 followed by A2), with the control limits taken from the first 24 points and extended unchanged. The four-dice baseline holds through Point 30; Point 31 drops onto the LCL as the dice change to two; from Point 37 the points jump above the UCL as the count rises to six."}
source(here::here("R/data/day-03-six-processes.R"))
individual_process_chart(six_processes, "A")
```
...
```{r chart-b3-individual, fig.width=9, fig.height=3.5, fig.align="center", fig.cap="Chart B3: numbers of Heads from 25-coin tosses across all 48 points, with the control limits taken from the first 24 and extended. Quiet inside the limits until around Point 40, then a steady upward trend as two extra coins are added at each toss from Point 39 onward."}
source(here::here("R/data/day-03-six-processes.R"))
individual_process_chart(six_processes, "B")
```
...
```{r chart-c3-individual, fig.width=9, fig.height=3.5, fig.align="center", fig.cap="Chart C3: Red Beads counts across all 48 paddles, with the control limits taken from the first 24 and extended. The familiar Red Beads pattern through Point 42, then the final five points jump well above the UCL — the contrived special cause was a measurement-recording change (summing the two junior inspectors' counts) for the last six paddles."}
source(here::here("R/data/day-03-six-processes.R"))
individual_process_chart(six_processes, "C")
```
...
```{r chart-d3-individual, fig.width=9, fig.height=3.5, fig.align="center", fig.cap="Chart D3: Neave's morning pulse rate over 48 consecutive days, with the control limits taken from the first 24 and extended. Stable but unhealthily high through Point 44, then a sharp drop to a new, stable, much lower level — the beta-blocker prescribed on day 44 acting as a 'good' special cause."}
source(here::here("R/data/day-03-six-processes.R"))
individual_process_chart(six_processes, "D")
```
...
[]{#sec-page32}
```{r chart-e3-individual, fig.width=9, fig.height=3.5, fig.align="center", fig.cap="Chart E3: daily averages of cigarette-lighter-socket measurements across 48 days (Tokai Rika case study), with the control limits taken from the first 24 and extended. Point 36 (Friday 26 September 1980) spikes upward; from Point 37 the line returns to target and the variability visibly tightens — Tokai Rika replaced the worn positioning collar the next working day."}
source(here::here("R/data/day-03-six-processes.R"))
individual_process_chart(six_processes, "E")
```
...
The data in Chart E3 are taken from an ancient but fascinating case study which Don Wheeler relates in a chapter on "Using Process Behaviour Charts for Continual Improvement" in his book with David Chambers: *Understanding Statistical Process Control*^b^. This is how Don introduces the story:
...
```{r chart-f3-individual, fig.width=9, fig.height=3.5, fig.align="center", fig.cap="Chart F3: monthly US trade deficits in billions of dollars across 2006–2009, with the control limits taken from the first 24 (2006–07) and extended. The deficit trends downward sharply after Point 31, drops below the LCL at Point 35 (November 2008, the first full month after the financial crisis), and stays below the LCL through the end of the chart."}
source(here::here("R/data/day-03-six-processes.R"))
individual_process_chart(six_processes, "F")
```
...
(a) Besides the Red Beads Experiment (Process C), Process B (counting the number of Heads when 25 coins were tossed) also fits the "batch inspection" conditions (see [page 13, in the How Do We Compute Those Control Limits section above](05-how-do-we-compute-control-limits.qmd#sec-page13)). So the method used for computing the control limits for Process B was the same as for the Red Beads Experiment except with $n$ = 25 instead of 50. The moving-range method was used for the other four processes.

(b) The control limits for all six processes were computed from 24 data. As mentioned earlier, this number was chosen simply because that was what we were used to using for the Red Beads Experiment. However, remember that for "live" charts a shorter baseline is recommended (Technical Aid 8 on [page 17, in the How Do We Compute Those Control Limits section above](05-how-do-we-compute-control-limits.qmd#sec-page17)).
