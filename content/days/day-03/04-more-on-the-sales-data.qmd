...
pagetitle: "Day 3: The 'sales data' were actually dice throws all along"
description: "The reveal: Day 2's monthly sales figures were really dice throws. A control chart shows the management's story-building was applied to pure noise."
session_minutes: 40
execute:
  echo: false
---
...
Does the Ford example from earlier today ([pages 5 to 6, at the Ford Motor Company](02-at-the-ford-motor-company.qmd#sec-page5)) suggest to you any concerns about the management's interpretation of yesterday's monthly sales data ([Day 2 pages 1–3](../day-02/01-run-charts.qmd#sec-page1)), the conclusions they drew, and the decisions they made?

```{ojs}
import { persist } from "../../../assets/scripts/workbook.js"

viewof thought_3f = persist(
  "content/days/day-03/04-more-on-the-sales-data.qmd#thought_3f",
  Inputs.textarea({label: "Concerns about management's interpretation of the sales data?", placeholder: "Type your comments here.", rows: 8})
)
```
...
Reveal suggested answer for Pause for Thought 3-f
</button>
...
::: {.column width="35%"}
```{r sales-control-chart, fig.width=5, fig.height=6, fig.cap="Sales data control chart: the same ten values upgraded with control limits (LCL = 6.7, UCL = 27.3) and a thin Central Line at 17. All ten points lie comfortably inside the limits — the process is in statistical control."}
run_chart_plot(
  c(13, 19, 18, 14, 16, 12, 21, 18, 17, 22),
  line_width = 4,
  y_limits = c(5, 30),
  y_breaks = seq(5, 30, by = 5),
  y_minor_breaks = seq(5, 30, by = 1),
  central_line = 17,
  hlines = c(6.7, 27.3)
)
```
...
<div class="centered-bold-callout">
