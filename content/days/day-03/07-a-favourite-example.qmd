...
pagetitle: "Day 3: 'Hugging the Central Line' — Deming's fear-driven fudge"
description: "Deming's favourite example: a control chart 'hugging the Central Line' too tightly reveals an inspector fudging figures out of fear of a plant closure."
session_minutes: 35
execute:
  echo: false
---
...
```{r favourite-example-run-chart, fig.width=8, fig.height=2.4, fig.align="center", fig.cap="Run chart of the daily proportion defective on final audit, over two months. The line wanders between roughly 8% and 9.6% — a tight band, with no control limits drawn yet."}
favourite_example <- read.csv("../../../R/data/day-03-favourite-example.csv", comment.char = "#")$proportion_defective
run_chart_plot(
  favourite_example, line_width = 3,
  y_limits = c(0.075, 0.10),
  y_breaks = c(0.08, 0.09, 0.10),
  y_minor_breaks = seq(0.08, 0.10, by = 0.001),
  gridlines = "none",
...
```{r favourite-example-control-chart, fig.width=8, fig.height=4, fig.align="center", fig.cap="The same daily proportions, redrawn on a much wider vertical scale. The correctly-computed control limits (LCL = 3.1%, UCL = 14.4%) are far apart, and every point clusters in a narrow band along the Central Line at 8.75% — the 'hugging the Central Line' pattern."}
run_chart_plot(
  favourite_example, line_width = 3,
  y_limits = c(0, 0.16),
  y_breaks = c(0, 0.05, 0.10, 0.15),
  y_minor_breaks = NULL,
  gridlines = "none",
  central_line = 0.0875,
  hlines = c(0.031, 0.144),
  hline_labels = c("LCL=3.1%", "UCL=14.4%"),
  hline_label_side = "left",
  hline_label_offset = c(-0.005, 0.005),
  y_label_fn = scales::label_percent(accuracy = 1),
  show_x_labels = FALSE
)
```
